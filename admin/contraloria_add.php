...
if (isset($_POST['add'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO disciplinarios (infraccion, correctivo, titulo, fecha, obs, pdf) VALUES (?, ?, ?, ?, ?, ?)");
    if ($stmt === false) {
        die('Error en la preparación de la consulta: ' . $conn->error);
    }

    // Enlazar los valores a los marcadores de posición
    $stmt->bind_param("ssssss", $infraccion,  $correctivo, $fecha ,  $obs, $filename);

    // Ejecutar la consulta preparada
    if ($stmt->execute()) {
        $_SESSION['success'] = 'Datos de la contraloria agregados satisfactoriamente';
    } else {
        $_SESSION['error'] = 'Error al agregar los datos: ' . $conn->error;
    }
    ...
    header('location: contraloria.php');
